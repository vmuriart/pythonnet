...
class AstParser(object):
    ...
    def get_struct_members(self, name):
        ...
        if name not in self.__struct_members:
            raise Exception("Unknown struct '{0!s}'".format(name))
    ...
    def __get_struct_name(self, node):
        return node.name or "_struct_{0:d}".format(id(node))
...
def gen_interop_code(members):
    ...
    defines = [
        "PYTHON{0:d}{1!s}".format(*(sys.version_info[:2]))
    ]
...
""".format(os.path.basename(__file__), " && ".join(defines))
    ...
    for name, tpy in members:
        ...
        class_definition += "        public static int {0!s} = 0;\n".format(name)
